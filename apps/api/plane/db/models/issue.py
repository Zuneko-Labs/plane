...
class IssueRecurrence(ProjectBaseModel):
    """Recurrence rule for a recurring work item.

    The source issue is the first occurrence, created at save time. A daily
    Celery Beat task (``generate_recurring_issues``) clones the live source
    issue into a new occurrence each time ``next_run_at`` falls due.
    """
    ...
    # For "N times per month" (semi-monthly) recurrences: the target days of the
    # month, e.g. [8, 23]. Empty means a single occurrence per period (stepped
    # by ``interval`` via add_interval).
    days_of_month = models.JSONField(default=list)
    # Recurrence runs indefinitely while is_active is True. Turning the toggle
    # off on the source work item (or any generated occurrence) stops future
    # occurrences.
    next_run_at = models.DateField(null=True, blank=True)
    is_active = models.BooleanField(default=True)
    # Work items generated by this recurrence. Lets the Recurring toggle appear
    # (and control the whole series) from any occurrence, not just the source.
    occurrences = models.ManyToManyField("db.Issue", related_name="+", blank=True)
    ...
    def __str__(self):
        return f"{self.source_issue_id} <{self.frequency}>"
